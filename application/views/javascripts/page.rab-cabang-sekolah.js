...
$(() => {
	...
	const refreshTotal = () => {
		...
		// view value
		$("#total_harga_ringgit").val('RM ' + window.apiClient.format.format_ringgit(ringgit_total.toFixed(2)))
		$("#total_harga_rupiah").val('Rp ' + window.apiClient.format.format_rupiah(rupiah_total.toFixed(2)))
	...
	$('#harga_ringgit').on('change', function () {
		...
		$.ajax({
			...
			$("#total_harga_rupiah").val('Rp ' + window.apiClient.format.format_rupiah(data.rupiah))
