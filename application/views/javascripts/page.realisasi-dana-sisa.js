			...
			"columns": [
				...
				},
				{
					"data": "total_harga_rupiah", render(data, type, full, meta) {
						return window.apiClient.format.format_rupiah(data)
					}
				},
				{
					"data": "harga_ringgit", render(data, type, full, meta) {
						returnwindow.apiClient.format.format_ringgit(data)
					}
				},
				{
		...
		$.ajax({
			...
			success: function (data) {
				if (data) {
					...
					$('#jumlah-total-ringgit').val('RM ' + window.apiClient.format.format_ringgit(data.total_harga_ringgit));
					$('#total-rupiah').val(data.total_harga_rupiah);
					$('#jumlah-total-rupiah').val('Rp ' + window.apiClient.format.format_rupiah(data.total_harga_rupiah));
					// jumlahkan
					$('#sisa-total-ringgit').val(Number(data.total_harga_ringgit) + Number($("#sisa-ringgit").val()));
					$('#jumlah-sisa-total-ringgit').val('RM ' + window.apiClient.format.format_ringgit(Number(data.total_harga_ringgit) + Number($("#sisa-ringgit").val())));
					$('#sisa-total-rupiah').val(Number(data.total_harga_rupiah) + Number($("#sisa-rupiah").val()));
					$('#jumlah-sisa-total-rupiah').val('Rp ' + window.apiClient.format.format_rupiah(Number(data.total_harga_rupiah) + Number($("#sisa-rupiah").val())));
...
const Tambah = (id) => {
	...
	$.ajax({
		...
		$.ajax({
			...
			$("#jumlah-sisa-ringgit").val('RM ' + window.apiClient.format.format_ringgit(data2.sisa_ringgit))
			$("#sisa-rupiah").val(data2.sisa_rupiah)
			$("#jumlah-sisa-rupiah").val('Rp ' + window.apiClient.format.format_rupiah(data2.sisa_rupiah))
		})
	...
	$('#jumlah-sisa-ringgit').on('change', function () {
		...
		$.ajax({
			...
			$("#jumlah-sisa-rupiah").val('Rp ' + window.apiClient.format.format_rupiah(data.rupiah))
		}).fail(($xhr) => {
			// console.log($xhr)
		})
...
function ubah(data) {
	if ($(data).attr("disabled") == undefined) {
		...
		$("#jumlah-sisa-ringgit").val('RM ' + window.apiClient.format.format_ringgit(jml_ringgit))
		$("#sisa-rupiah").val(jml_rupiah)
		$("#jumlah-sisa-rupiah").val('Rp ' + window.apiClient.format.format_rupiah(jml_rupiah))
		$('#myModalLabel').html('Alihkan Dana Sisa')

		// non rab
		$("#val_harga_ringgit").val(jml_ringgit)
		$("#harga_ringgit").val('RM ' + window.apiClient.format.format_ringgit(jml_ringgit))
		$("#val_harga_rupiah").val(jml_rupiah)
		$("#harga_rupiah").val('RM ' + window.apiClient.format.format_rupiah(jml_rupiah))
		// total non rab
		$("#val_total_harga_ringgit").val(jml_ringgit)
		$("#total_harga_ringgit").val('RM ' + window.apiClient.format.format_ringgit(jml_ringgit))
		$("#val_total_harga_rupiah").val(jml_rupiah)
		$("#total_harga_rupiah").val('RM ' + window.apiClient.format.format_rupiah(jml_rupiah))
		$('#myModal').modal('toggle')
	} else {
