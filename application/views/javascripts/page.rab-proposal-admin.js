			...
			"columns": [
				...
				},
				{
					"data": "id", render(data, type, full, meta) {
						// cek status
						const btndetail = `
						<button class="btn btn-primary btn-xs" data-id_cabang="${full.id_cabang}" data-id="${data}" data-judul="${full.judul}" data-keterangan="${full.keterangan}" data-ringgit="${full.total_ringgit}" data="${full.id}" data-status="1" onclick="Ubah(this)">
...
const Ubah = (dataset) => {
	...
	$.ajax({
		...
		data.forEach(e => {
			...
			strbody += `
			<tr>
			${status == 0 ? `<td><input type="checkbox" class="check" data-id="${e.id}" data-ringgit="${e.total_harga_ringgit}" data-rupiah="${e.total_harga_rupiah}" ${e.ischeck == 0 ? "" : "checked"} id="check-${e.id}"></td>` : ''}
			<td>${e.kode}</td>
			<td>${e.nama}</td>
